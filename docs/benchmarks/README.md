...
python3 -m unittest discover -s scripts -p 'test_quality_benchmark.py'
...
The synthetic command intentionally exits 2 for the four regressions, after
writing its report. For real checks, clone the named repositories into the
`directory` names in the manifest under a common parent, check out each exact
revision, then add `--corpus tests/fixtures/quality/real.json --real-root PARENT`.
The harness refuses dirty or wrong-revision real checkouts. It makes no network
requests, executes no fixture/project code, and calls no models.

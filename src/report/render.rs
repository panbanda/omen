...
impl Renderer {
    ...
    fn load_data(&self, data_dir: &Path) -> Result<RenderData> {
        ...
        // Generate programmatic summary if LLM insights are not available
        if data.summary.is_none() {
            data.summary = Some(generate_programmatic_summary(&data));
        }
...
/// Generate a programmatic summary based on analysis data when LLM insights are unavailable.
fn generate_programmatic_summary(data: &RenderData) -> SummaryInsight {
    let mut summary = SummaryInsight::default();

    // Generate executive summary based on score
    let score_val = data.score.score;
    let grade = if score_val >= 90 {
        "A"
    } else if score_val >= 80 {
        "B"
    ...
    summary.executive_summary = format!(
        "This repository has a health score of **{}/100** (Grade: **{}**), indicating {} code health. \
        The analysis examined {} files.",
        score_val, grade, health_status, data.score.files_analyzed
    );

    // Count critical issues (components with score < 50)
    let critical_count = data.score.components.values().filter(|&&s| s < 50).count();
    if critical_count > 0 {
        summary.executive_summary.push_str(&format!(
    ...
    // Hotspot findings
    if let Some(ref hotspots) = data.hotspots {
        let count = hotspots.files.len();
        if count > 0 {
            findings.push(format!(
                "**{}** hotspot files identified with high complexity and frequent changes.",
                count
            ));
        }
    }
    ...
    // SATD findings
    if let Some(ref satd) = data.satd {
        let total = satd.items.len();
        if total > 0 {
            let critical_count = satd
                .items
                .iter()
                .filter(|i| i.severity == "Critical")
                .count();
            let high_count = satd.items.iter().filter(|i| i.severity == "High").count();
            if critical_count > 0 || high_count > 0 {
                findings.push(format!(
                    "**{}** self-admitted technical debt items found ({} critical, {} high priority).",
                    total, critical_count, high_count
                ));
            } else {
                findings.push(format!(
                    "**{}** self-admitted technical debt items found.",
                    total
                ));
            }
        }
    ...
    if let Some(ref duplicates) = data.duplicates {
        if duplicates.clone_groups > 0 {
            findings.push(format!(
                "**{}** code clone groups detected with **{:.1}%** duplication ratio.",
                duplicates.clone_groups, duplicates.duplication_ratio
            ));
        }
    }

    // Ownership findings
    if let Some(ref ownership) = data.ownership {
        if ownership.summary.bus_factor <= 2 {
            findings.push(format!(
                "Low bus factor of **{}** indicates knowledge concentration risk.",
                ownership.summary.bus_factor
            ));
        }
        if ownership.summary.silo_count > 0 {
            findings.push(format!(
                "**{}** knowledge silos detected.",
                ownership.summary.silo_count
            ));
        }
    }

    // Feature flags findings
    if let Some(ref flags) = data.flags {
        let stale_count = flags
            .flags
            .iter()
            .filter(|f| f.stale.unwrap_or(false))
            .count();
        if stale_count > 0 {
            findings.push(format!(
                "**{}** stale feature flags should be cleaned up.",
                stale_count
            ));
        }
    }
    ...
    for (name, &score_value) in &data.score.components {
        let component_name = format_component_name(name);
        if score_value < 50 {
            high_priority.push(Recommendation {
                title: component_name.clone(),
                description: format!("Score of {}/100 requires immediate attention.", score_value),
            });
        } else if score_value < 70 {
            medium_priority.push(Recommendation {
                title: component_name.clone(),
                description: format!("Score of {}/100 could be improved.", score_value),
            });
        } else if score_value < 90 {
            ongoing.push(Recommendation {
                title: component_name.clone(),
                description: format!("Maintain current practices (score: {}/100).", score_value),
            });
        }
    }
...
fn format_component_name(name: &str) -> String {
    match name {
        "satd" => "Known Debt".to_string(),
        "tdg" => "Debt Gradient".to_string(),
        "duplication" => "Code Duplication".to_string(),
        _ => name
            .chars()
            .enumerate()
            .map(|(i, c)| {

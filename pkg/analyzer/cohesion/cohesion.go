...
// Analyze computes CK metrics for all OO classes in the project.
...
func (a *Analyzer) Analyze(ctx context.Context, files []string) (*Analysis, error) {
	...
	inheritanceTree := a.buildInheritanceTreeWithContext(ctx, files)

	// Second pass: Process files in parallel to extract CK metrics
	results, _ := fileproc.MapFiles(ctx, files, func(psr *parser.Parser, path string) ([]ClassMetrics, error) {
...
// buildInheritanceTreeWithContext scans all files to build the inheritance graph.
func (a *Analyzer) buildInheritanceTreeWithContext(ctx context.Context, files []string) *inheritanceTree {
	...
	results, _ := fileproc.MapFiles(ctx, files, func(psr *parser.Parser, path string) ([]inheritanceInfo, error) {
		if a.skipTestFile && isTestFile(path) {
			return nil, nil
		}

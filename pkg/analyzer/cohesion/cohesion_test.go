...
func TestNewWithOptions(t *testing.T) {
	a := New(WithIncludeTestFiles(), WithMaxFileSize(1024))
	defer a.Close()
...
func TestIsTestFile(t *testing.T) {
	tests := []struct {
		...
		{"calculator_test.go", true},
		{"calculator_test.py", true},
		{"calculator.test.ts", true},
		{"calculator.test.js", true},
		{"calculator.spec.ts", true},
		{"calculator.spec.js", true},
		{"/path/to/test/calculator.py", true},
		{"/path/to/tests/calculator.py", true},
		{"/path/to/__tests__/calculator.js", true},
		{"calculator.go", false},
		{"calculator.py", false},
		{"calculator.ts", false},
	}
...
func TestAnalysis_SortMethods(t *testing.T) {
	analysis := &Analysis{
		Classes: []ClassMetrics{
			{ClassName: "A", LCOM: 5, WMC: 10, CBO: 8, DIT: 2},
			{ClassName: "B", LCOM: 3, WMC: 20, CBO: 3, DIT: 5},
			{ClassName: "C", LCOM: 8, WMC: 5, CBO: 15, DIT: 1},
		},
	}

	// Test SortByLCOM
	analysis.SortByLCOM()
	if analysis.Classes[0].ClassName != "C" {
		t.Errorf("After SortByLCOM, first class should be C (LCOM=8), got %s", analysis.Classes[0].ClassName)
	}

	// Test SortByWMC
	analysis.SortByWMC()
	if analysis.Classes[0].ClassName != "B" {
		t.Errorf("After SortByWMC, first class should be B (WMC=20), got %s", analysis.Classes[0].ClassName)
	}

	// Test SortByCBO
	analysis.SortByCBO()
	if analysis.Classes[0].ClassName != "C" {
		t.Errorf("After SortByCBO, first class should be C (CBO=15), got %s", analysis.Classes[0].ClassName)
	}

	// Test SortByDIT
	analysis.SortByDIT()
	if analysis.Classes[0].ClassName != "B" {
		t.Errorf("After SortByDIT, first class should be B (DIT=5), got %s", analysis.Classes[0].ClassName)
	}
}
...
func TestAnalysis_CalculateSummary(t *testing.T) {
	analysis := &Analysis{
		Classes: []ClassMetrics{
			{Path: "a.py", ClassName: "A", WMC: 10, CBO: 5, RFC: 15, LCOM: 2, DIT: 1},
			{Path: "b.py", ClassName: "B", WMC: 20, CBO: 8, RFC: 25, LCOM: 3, DIT: 2},
			{Path: "a.py", ClassName: "C", WMC: 5, CBO: 3, RFC: 10, LCOM: 1, DIT: 0},
		},
	}
	...
	if analysis.Summary.MaxWMC != 20 {
		t.Errorf("MaxWMC = %d, want 20", analysis.Summary.MaxWMC)
	}
	if analysis.Summary.MaxCBO != 8 {
		t.Errorf("MaxCBO = %d, want 8", analysis.Summary.MaxCBO)
	}
	if analysis.Summary.MaxRFC != 25 {
		t.Errorf("MaxRFC = %d, want 25", analysis.Summary.MaxRFC)
	}
	if analysis.Summary.MaxLCOM != 3 {
		t.Errorf("MaxLCOM = %d, want 3", analysis.Summary.MaxLCOM)
	}
	if analysis.Summary.MaxDIT != 2 {
		t.Errorf("MaxDIT = %d, want 2", analysis.Summary.MaxDIT)
	}
	// LCOM > 1 counts: A (2) and B (3)
	if analysis.Summary.LowCohesionCount != 2 {
		t.Errorf("LowCohesionCount = %d, want 2", analysis.Summary.LowCohesionCount)
	}
}

...
func TestDefaultWeights_SumToOne(t *testing.T) {
	w := DefaultWeights()
	sum := w.Complexity + w.Duplication + w.Defect + w.SATD + w.TDG + w.Coupling + w.Smells + w.Cohesion
...
func TestResult_ComputeComposite(t *testing.T) {
	...
	// 80*0.20 + 90*0.15 + 70*0.20 + 60*0.10 + 75*0.10 + 85*0.10 + 95*0.05 + 80*0.10
	// = 16 + 13.5 + 14 + 6 + 7.5 + 8.5 + 4.75 + 8 = 78.25 -> 78
	if r.Score < 76 || r.Score > 80 {

...
func (a *Analyzer) AnalyzeProjectFromSource(files []string, src ContentSource, commitHash string) (*Result, error) {
	...
	wg.Go(func() {
		cxResult, _ = complexityAnalyzer.AnalyzeProjectFromSource(files, src)
	})
	wg.Go(func() {
		dupResult, _ = duplicatesAnalyzer.AnalyzeProjectFromSource(files, src)
	})
	wg.Go(func() {
	...
	wg.Go(func() {
		cohesionResult, _ = cohesionAnalyzer.AnalyzeProjectFromSource(files, src)
	})
